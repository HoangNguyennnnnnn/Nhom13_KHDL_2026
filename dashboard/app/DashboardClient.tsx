...
export default function DashboardClient({
  ...
  const [simulatorBrand, setSimulatorBrand] = useState(elasticityBrands[0] || "");
  const [discountIncrement, setDiscountIncrement] = useState(5);

  // Tab 4: XGBoost Simulator
  const [xgbProdId, setXgbProdId] = useState(products[0]?.Product_ID || "iphone-17-pro-max");
  ...
  const [compareSortBy, setCompareSortBy] = useState<"name" | "brand" | "price" | "ramrom" | "rating" | "reviews">("price");
  ...
  const handleForecast = async () => {
    ...
    try {
      ...
      setForecastError(`Lỗi backend dự báo: ${message}. Hãy kiểm tra API FastAPI ở cổng 8000.`);
    } finally {
  ...
  const handlePredictSentiment = async () => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000/api/v1/sentiment/predict", {
        method: "POST",
      ...
      if (response.ok) {
        ...
        setSentimentError("Không thể kết nối đến backend API. Hãy đảm bảo FastAPI đã khởi động thành công!");
      }
    } catch (err) {
  ...
  const handleCompareSort = (key: "name" | "brand" | "price" | "ramrom" | "rating" | "reviews") => {
  ...
  const getSortIndicator = (key: "name" | "brand" | "price" | "ramrom" | "rating" | "reviews") => {
    if (compareSortBy !== key) return "↕";
    return compareSortDir === "asc" ? "↑" : "↓";
  };

  // Filter products by brand and segment
  const brandFilteredProducts = selectedBrand === "Tất cả"
    ? products
  ...
  const filteredProducts = selectedSegment === "Tất cả"
    ? brandFilteredProducts
    : brandFilteredProducts.filter(p => (p.cluster_name || "Không xác định") === selectedSegment);
    ...
    <div className="dashboard-container">
      ...
      <aside className="sidebar">
        <div className="brand-logo">
          <img src="https://img.icons8.com/clouds/200/smart-phone.png" alt="logo" width="64" />
          <h2>TGDD Smart Analytics</h2>
        </div>
        <nav className="nav-menu">
          <button className={activeTab === "phankhuc" ? "active" : ""} onClick={() => setActiveTab("phankhuc")}>
            🎯 Phân Khúc K-Means & PCA
          </button>
          <button className={activeTab === "sentiment" ? "active" : ""} onClick={() => setActiveTab("sentiment")}>
            💬 Ý Kiến Khách Hàng (Aspect)
          </button>
          <button className={activeTab === "danhygia" ? "active" : ""} onClick={() => setActiveTab("danhygia")}>
          ...
          <button className={activeTab === "sentiment_room" ? "active" : ""} onClick={() => setActiveTab("sentiment_room")}>
            🤖 ML Sentiment Room
          </button>
          <button className={activeTab === "teencode" ? "active" : ""} onClick={() => setActiveTab("teencode")}>
            📚 Từ Điển Teen Code
          </button>
        </nav>
      </aside>
      ...
      <main className="main-viewport">
        ...
        {activeTab === "phankhuc" && (
          <div className="tab-pane animate-fade">
            ...
            <div className="explanation-box">
              ...
              <ul>
                <li><b>Trục Ngang (PC1):</b> Đại diện cho <b>Cấu hình & Giá tiền</b>. Càng đi sang phải, máy càng đắt, RAM/ROM khủng và camera xịn (Cao cấp).</li>
                <li><b>Trục Đứng (PC2):</b> Đại diện cho <b>Sức mua & Lượt tương tác</b>. Càng lên cao, máy càng có doanh số bán ra lớn từ Thế Giới Di Động.</li>
                <li><b>Mỗi Màu sắc:</b> Đại diện cho một phân khúc giá và cấu hình rõ ràng, giúp so sánh đối thủ cạnh tranh trực tiếp một cách công bằng nhất.</li>
              </ul>
            </div>
            ...
            <div className="chart-wrapper">
              <ResponsiveContainer width="100%" height={400}>
                <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  ...
                  <XAxis type="number" dataKey="x" name="PC1" label={{ value: 'Độ Cao Cấp & Giá Bán (PC1)', position: 'bottom', offset: 0 }} />
                  <YAxis type="number" dataKey="y" name="PC2" label={{ value: 'Sức Mua & Lượt Đánh Giá (PC2)', angle: -90, position: 'left' }} />
                  <Tooltip
                      ...
                      if (active && payload && payload.length) {
                        ...
                        return (
                          <div className="custom-tooltip">
                            ...
                            <p>RAM: {data.ram} GB | ROM: {data.rom} GB</p>
                          </div>
                        );
                      }
                      return null;
                  ...
                  <Scatter name="Điện thoại" data={scatterData}>
                    {scatterData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={clusterColors[entry.cluster] || "#4a5568"} />
                    ))}
                  </Scatter>
                </ScatterChart>
              </ResponsiveContainer>
            ...
            <div className="table-wrapper" style={{ marginTop: "16px" }}>
              ...
              {selectedSegment === "Tất cả" ? (
                <p>Chọn một phân khúc ở bộ lọc phía trên để mở bảng so sánh trực tiếp trong cùng nhóm.</p>
              ) : (
                <table>
                  <thead>
                    <tr>
                      <th style={{ cursor: "pointer" }} onClick={() => handleCompareSort("name")}>Sản phẩm {getSortIndicator("name")}</th>
                      <th style={{ cursor: "pointer" }} onClick={() => handleCompareSort("brand")}>Thương hiệu {getSortIndicator("brand")}</th>
                      <th style={{ cursor: "pointer" }} onClick={() => handleCompareSort("price")}>Giá {getSortIndicator("price")}</th>
                      <th style={{ cursor: "pointer" }} onClick={() => handleCompareSort("ramrom")}>RAM/ROM {getSortIndicator("ramrom")}</th>
                      <th style={{ cursor: "pointer" }} onClick={() => handleCompareSort("rating")}>Điểm sao {getSortIndicator("rating")}</th>
                      <th style={{ cursor: "pointer" }} onClick={() => handleCompareSort("reviews")}>Tổng đánh giá {getSortIndicator("reviews")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {comparedRows.map((p) => (
                        <tr key={`cmp-${p.Product_ID}`}>
                          ...
                          <td>{p.Brand || "N/A"}</td>
                          <td>{Number(p.Discounted_Price || 0).toLocaleString("vi-VN")} đ</td>
                          <td>{p.RAM ?? "?"}GB / {p.ROM ?? "?"}GB</td>
                          <td>{Number(p.Avg_Star_Rating || 0).toFixed(1)}</td>
                          <td>{Number(p.Total_Reviews || 0).toLocaleString("vi-VN")}</td>
                        </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <tbody>
                  {filteredProducts.map(p => (
                    <tr key={p.Product_ID}>
                      ...
                      <td>{p.Brand}</td>
                      <td><span className="price-tag">{p.Discounted_Price?.toLocaleString("vi-VN")} đ</span></td>
                      <td>{p.RAM}GB / {p.ROM}GB</td>
                      <td>
                        <span className="segment-badge" style={{ backgroundColor: clusterColors[p.cluster_name || ""] + "1A", color: clusterColors[p.cluster_name || ""] }}>
                          ● {p.cluster_name}
                        </span>
                      </td>
                    </tr>
        ...
        {activeTab === "sentiment" && (
          <div className="tab-pane animate-fade">
            <div className="pane-header">
              <h2>💬 Phân Tích Ý Kiến Người Dùng & Ưu/Nhược Điểm Từng Sản Phẩm (Pros & Cons)</h2>
              <div className="filters">
                <label>Chọn sản phẩm: </label>
                <select value={selectedAspectProductKey} onChange={(e) => setSelectedAspectProductKey(e.target.value)}>
                  {aspectProductKeys.map(k => (
                    <option key={k} value={k}>{aspects[k].Product_Name}</option>
                  ))}
                </select>
              </div>
            </div>
              ...
              <>
                ...
                <div className="chart-wrapper">
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={aspectChartData} layout="vertical" margin={{ left: 30, right: 30 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#edf2f7" />
                      <XAxis type="number" domain={[0, 100]} />
                      <YAxis type="category" dataKey="name" />
                      <Tooltip formatter={(value) => [`${Number(value).toFixed(1)}%`, 'Độ hài lòng']} />
                      <Bar dataKey="Score" radius={[0, 4, 4, 0]}>
                        {aspectChartData.map((entry, index) => {
                          const color = entry.Score >= 60 ? "#48bb78" : (entry.Score >= 45 ? "#ecc94b" : "#f56565");
                    ...
                    <ul>
                      {Object.keys(selectedProdData.Aspects).map(aspect => {
                        ...
                        if (details.Score >= 0.6) {
                          return (
                            <li key={aspect}>
                              <strong>{aspect} ({Math.round(details.Score * 100)}%):</strong>
                              <p className="quote">“{details.Pros[0] || "Khách hàng rất hài lòng về mặt này."}”</p>
                            </li>
                          );
                        }
                        return null;
                      ...
                      {Object.keys(selectedProdData.Aspects).map(aspect => {
                        ...
                        if (details.Score < 0.45) {
                          return (
                            <li key={aspect}>
                              <strong>{aspect} ({Math.round(details.Score * 100)}%):</strong>
                              <p className="quote negative">“{details.Cons[0] || "Ghi nhận một số phàn nàn nhẹ."}”</p>
                            </li>
                          );
                        }
                        return null;
            ...
            <div className="chart-wrapper">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={elasticityChartData}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Bar dataKey="Độ co giãn" fill="#319795" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="panel promo-simulator">
              ...
              <div className="simulator-grid">
                <div className="controls">
                  ...
                  <select value={simulatorBrand} onChange={(e) => setSimulatorBrand(e.target.value)}>
                  ...
                  <label style={{ marginTop: "16px", display: "block" }}>
                    2. Tỷ lệ tăng thêm chiết khấu dự kiến (%): <strong>+{discountIncrement}%</strong>
                  </label>
                  <input
                    type="range"
                    min="1"
                    ...
                    onChange={(e) => setDiscountIncrement(Number(e.target.value))}
                    className="slider"
                  />
                </div>
                ...
                <div className="results">
                  {elasticity[simulatorBrand] ? (
                    (() => {
                      ...
                      const directionWord = growthPercent >= 0 ? "tăng" : "giảm";
                      const growthSign = growthPercent >= 0 ? "+" : "-";
                      return (
                        <div className="alert-success">
                          <h4>🔮 Ước tính Kết quả cho {simulatorBrand}:</h4>
                          <ul>
                            <li>Độ nhạy giá (Elasticity Coefficient): <code>{elas.toFixed(4)}</code></li>
                            <li>Doanh số TB hiện tại: <code>{avgSales.toLocaleString("vi-VN")} máy</code></li>
                            <li>Lượng hàng bán ra dự kiến {directionWord}: <strong className="highlight">{growthSign}{Math.abs(addedSales).toFixed(1)} máy</strong></li>
                            <li>Tỷ lệ tăng trưởng doanh số: <strong className="highlight">{growthSign}{Math.abs(growthPercent).toFixed(2)}%</strong></li>
                          </ul>
                        </div>
                      );
                    })()
        ...
        {activeTab === "forecasting" && (
          <div className="tab-pane animate-fade">
            <div className="pane-header">
              <h2>🔮 Mô Hình Học Máy Dự Báo Doanh Số Smartphone Tương Lai (XGBoost)</h2>
            </div>
            ...
            <div className="forecast-simulator-panel">
              <div className="form-column">
                ...
                <select value={xgbProdId} onChange={(e) => setXgbProdId(e.target.value)}>
                  {products.map(p => (
                ...
                <button className="btn-predict" onClick={handleForecast} disabled={forecasting}>
                  {forecasting ? "⌛ Đang tính toán..." : "🔮 Chạy Dự Báo 7 Ngày"}
                </button>
                {forecastError && <p style={{ color: "#e53e3e", marginTop: 12 }}>{forecastError}</p>}
              </div>
              ...
              <div className="chart-column">
                ...
                {forecastResults.length > 0 ? (
                  <>
                    <ResponsiveContainer width="100%" height={250}>
                      <LineChart data={forecastResults}>
                        ...
                        <YAxis />
                        <Tooltip />
                        <Line type="monotone" dataKey="predicted_volume" name="Lượng máy bán ra" stroke="#e43f5a" strokeWidth={3} />
                      </LineChart>
                    </ResponsiveContainer>
                      ...
                      <table>
                        ...
                        <tbody>
                          {forecastResults.map(row => (
                            <tr key={row.date}>
                              <td>{row.date}</td>
                              <td><strong>{row.predicted_volume} máy</strong></td>
                              <td>
                                <span className={`trend-badge ${row.trend}`}>
                                  {row.trend === "up" ? "▲ Tăng" : row.trend === "down" ? "▼ Giảm" : "● Ổn định"}
                                </span>
                              </td>
                            </tr>
                          ))}
                  ...
                  <div className="placeholder-forecast">
                    <p>Nhấp nút <b>"Chạy Dự Báo 7 Ngày"</b> để xem đồ thị trực quan và bảng số liệu phân tích của mô hình XGBoost.</p>
                  </div>
                )}
              </div>
            </div>
        ...
        {activeTab === "sentiment_room" && (
          <div className="tab-pane animate-fade">
            <div className="pane-header">
              <h2>🤖 Trải Nghiệm Mô Hình Nhận Diện Ý Kiến Đánh Giá (TF-IDF + Random Forest)</h2>
            </div>

            <div className="explanation-box">
              <h4>💡 Giải thích trực quan về mặt Khoa học Dữ liệu (NLP):</h4>
              <ul>
                <li>Bình luận bạn nhập sẽ tự động chạy qua bộ dịch **Teen code & viết tắt** để chuẩn hoá từ ngữ.</li>
                <li>Hệ thống chạy qua bộ trích xuất đặc trưng **TF-IDF** và đưa vào mô hình học máy **Random Forest Classifier** đã huấn luyện để phân tích ý kiến chính xác.</li>
              </ul>
            </div>

            <div className="teencode-tool-grid" style={{ gridTemplateColumns: "1fr" }}>
              <div className="translator-panel">
                ...
                <textarea
                  ...
                  rows={5}
                />
                <button className="btn-predict" onClick={handlePredictSentiment} disabled={sentimentLoading} style={{ marginTop: "16px" }}>
                  {sentimentLoading ? "⌛ Đang phân tích cảm xúc..." : "🔥 Phân Tích Cảm Xúc ML"}
                </button>
                {sentimentError && <p style={{ color: "#e53e3e", marginTop: 12 }}>{sentimentError}</p>}

                {sentimentResult && (
                  <div className="translation-result animate-fade" style={{ marginTop: "24px" }}>
                    <div style={{ display: "flex", gap: "20px", marginBottom: "16px" }}>
                      <div style={{ flex: 1, backgroundColor: "#fff", padding: "12px", borderRadius: "6px", borderLeft: "4px solid #3182ce" }}>
                        <strong>Văn bản gốc:</strong>
                        <p style={{ fontStyle: "italic", marginTop: "4px", color: "#4a5568" }}>“{sentimentInput}”</p>
                      </div>
                      <div style={{ flex: 1, backgroundColor: "#fff", padding: "12px", borderRadius: "6px", borderLeft: "4px solid #38a169" }}>
                        <strong>Đã dịch Teen code & chuẩn hoá:</strong>
                        <p style={{ fontStyle: "italic", marginTop: "4px", color: "#2f855a" }}>“{sentimentResult.cleaned_text}”</p>
                      </div>
                    </div>

                    <div style={{ padding: "20px", borderRadius: "8px", backgroundColor: sentimentResult.label === 1 ? "#f0fff4" : "#fff5f5", border: `1px solid ${sentimentResult.label === 1 ? "#c6f6d5" : "#fed7d7"}`, textAlign: "center" }}>
                      {sentimentResult.label === 1 ? (
                        <h3 style={{ color: "#22543d" }}>🎉 Kết Quả: TÍCH CỰC (Positive)</h3>
                      ) : (
                        <h3 style={{ color: "#742a2a" }}>⚠️ Kết Quả: TIÊU CỰC / GÓP Ý (Negative)</h3>
                      )}
                      
                      <div style={{ marginTop: "16px" }}>
                        <span style={{ fontWeight: "700", fontSize: "14px", color: "#4a5568" }}>
                          Độ tin cậy của mô hình Random Forest: {(sentimentResult.confidence * 100).toFixed(2)}%
                        </span>
                        <div style={{ width: "100%", height: "12px", backgroundColor: "#edf2f7", borderRadius: "6px", overflow: "hidden", marginTop: "8px" }}>
                          <div style={{ width: `${sentimentResult.confidence * 100}%`, height: "100%", backgroundColor: sentimentResult.label === 1 ? "#48bb78" : "#e53e3e" }} />
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="teencode-tool-grid">
              <div className="translator-panel">
                ...
                <textarea
                  ...
                  rows={4}
                />
                <button className="btn-translate" onClick={handleTranslate} style={{ marginTop: "12px" }}>Dịch Dữ Liệu ⚡</button>

                {translatedText && (
                  <div className="translation-result animate-fade" style={{ marginTop: "16px" }}>
