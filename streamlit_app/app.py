...
with tab2:
    ...
    if not aspects_dict:
        ...
        with col_weakness:
            st.markdown("<div style='background-color:#f8d7da; padding: 15px; border-radius: 8px; border-left: 5px solid #dc3545;'><h4 style='color:#721c24; margin:0 0 10px 0;'>ĐIỂM YẾU / HẠN CHẾ (Weaknesses)</h4>", unsafe_allow_html=True)
        ...
        for aspect_name, details in prod_data["Aspects"].items():
            ...
            with st.expander(f"Khía cạnh {aspect_name} ({sentiment_indicator} - {score_val*100:.1f}%)"):
...
with tab3:
    ...
    if not elasticity_dict:
        ...
        st.markdown("### Trình mô phỏng Kế hoạch Khuyến mãi thông minh")
...
with tab4:
    ...
    if not model_path.exists():
        ...
        st.subheader("Thử nghiệm Dự báo Doanh số sản phẩm")
    ...
    if st.button("Phân tích ý kiến", type="primary"):
        if not model_dir.exists():
            ...
            if name == "positive":
                ...
                st.warning(f"### Nhãn: Trung tính (Neutral) - Độ tin cậy: {confidence:.2%}")
            else:
                st.error(f"### Nhãn: Tiêu cực / Góp ý (Negative) - Độ tin cậy: {confidence:.2%}")
            ...
            for col, key, vi in zip(cols, ["negative", "neutral", "positive"], ["Tiêu cực", "Trung tính", "Tích cực"]):
                if key in prob_map:
                    col.metric(vi, f"{prob_map[key] * 100:.1f}%")
